...
\begin{verbatim}
tree fa arbor drzewo 709 N plant, HAS material[wood], HAS trunk/2759, HAS crown 
\end{verbatim}
...
as is. The (historically) prevalant mode of traveling, on horesback, is kept
as a default. Note that these two entries often get translated by different
words: for example Hungarian distinguishes {\it utazik} and {\it lovagol}. 

\subsection{More complex notation}

Several more complex notations are used now, in particular parentheses {\tt
  ()} node-fillers like {\tt =AGT, =PAT}, but most of these will be
eliminated. Therefore, it is reasonable to summarize what we have so far in a
