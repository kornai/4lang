...
seem gen think[=AGT IS_A =PAT], "dative" MARK =PAT
...
The only relations countenanced in 4lang are binaries (see
\citet{Kornai:2012} for how ditransitive and higher arity verbs can be
treated). A typical example would be AT, which means `=AGT is located
where =PAT is'. Since temporal locations (at 3PM), and abstract state
(at ease) are commonly understood, we treat all of these as
manifestations of one and the same abstract AT. 
...
Systems of Knowledge Representation (KR) such as Cyc
\citep{Lenat:1990} often add `glue ret is common to distinguish individuals, e.g. some
particular poet, say Allan Ginsberg, from the class Poet, of which
Ginsberg is an {\sc InstanceOf}. In 4lang, typing is kept to the
absolute minimum, and no glue relations are used. That said, there
remain a few irreducible relations,
...
the subject is collective: %{\it close, between, through, 





x PART_OF y = y HAS_PART x 67

x INSRUMENT_OF y = y HAS_INSTRUMENT x

x ABOVE y = y BELOW x

x BEFORE y = y AFTER x  4 (can be eliminated!!) (AFTER 0) 

x IS_ABOUT  y has no obverse? 12

x AT y =? y AT x 139

x IS_A 26  y antisymmetric, definable; LACK 88 partially negates it 

string_in_context MARK thought, emotion, grammar 65
...
CAUSE 127 metal fatigue can cause deadly accidents \neg\rightarrow deadly accidents are caused by metal fatigue
ER > 50
FOLLOW 27
FOR 28
FROM 13
